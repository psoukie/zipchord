...
zc_lookup_count := DllCall("GetProcAddress", "Ptr", hZipChord, "AStr", "zc_lookup_count", "Ptr")
...
QPC()
written := DllCall(zc_lookup_chord, "AStr", chord, "Ptr", &outBuf, "Int", bufSize, "Cdecl Int")
QPC()

if (written > 0) {
    expansion := StrGet(&outBuf, "UTF-8")
